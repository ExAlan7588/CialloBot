...
from utils.localization import get_localized_string as lstr # Assuming you might use localization later
...
class ModSelect(discord.ui.Select):
    def __init__(self, parent_view, available_mods):
        ...
        if not options:
            options = [discord.SelectOption(label="No Mods Available", value="_disabled", default=True)]
            super().__init__(placeholder="No mods applicable...", min_values=0, max_values=1, options=options, disabled=True)
        else:
            super().__init__(placeholder="Select mods...", min_values=0, max_values=len(available_mods), options=options)

    async def callback(self, interaction: discord.Interaction):
        await self.parent_view.update_embed_with_mods(interaction, self.values)

class ModSelectView(discord.ui.View):
    def __init__(self, cog_instance, beatmap_id: int, current_ruleset_id: int, target_beatmap: dict, beatmapset_data: dict, user_id_for_l10n: str, all_maps_in_set: list | None = None, current_difficulty_index: int | None = None):
        ...
        if self.all_maps_in_set and self.current_difficulty_index is not None:
            ...
            self.prev_difficulty_button = discord.ui.Button(label=prev_label, style=discord.ButtonStyle.secondary, row=1)
            ...
            self.next_difficulty_button = discord.ui.Button(label=next_label, style=discord.ButtonStyle.secondary, row=1)
    ...
    def _update_pagination_buttons_state(self):
        if not self.all_maps_in_set or self.current_difficulty_index is None:
            if hasattr(self, 'prev_difficulty_button'): self.prev_difficulty_button.disabled = True
            if hasattr(self, 'next_difficulty_button'): self.next_difficulty_button.disabled = True
            return
    ...
    async def prev_difficulty_callback(self, interaction: discord.Interaction):
        if self.all_maps_in_set and self.current_difficulty_index is not None and self.current_difficulty_index > 0:
    ...
    async def next_difficulty_callback(self, interaction: discord.Interaction):
        if self.all_maps_in_set and self.current_difficulty_index is not None and self.current_difficulty_index < len(self.all_maps_in_set) - 1:
            self.current_difficulty_index += 1
            await self._update_difficulty(interaction)
    ...
    async def _update_difficulty(self, interaction: discord.Interaction):
        ...
        ruleset_id_map = {"osu": 0, "taiko": 1, "fruits": 2, "mania": 3} # Duplicated from pp method, maybe move to class/cog level
        ...
        if not new_attributes or "attributes" not in new_attributes:
            await interaction.followup.send(
                lstr(self.user_id_for_l10n, "error_beatmap_attributes_not_found", "Could not retrieve beatmap attributes for the new difficulty."),
                ephemeral=True
            )
            return
    ...
    async def update_embed_with_mods(self, interaction: discord.Interaction, selected_mods: list[str]):
        ...
        if not new_attributes or "attributes" not in new_attributes:
            ...
            try:
                await interaction.followup.send(
                    lstr(self.user_id_for_l10n, "error_beatmap_attributes_not_found", "Could not retrieve beatmap attributes with selected mods."),
                    ephemeral=True
                )
            except discord.HTTPException:
                 logger.error(f"[PP_COG] Failed to send followup for attribute error after mod selection. Interaction: {interaction.id}")
            return
...
class PpCog(commands.Cog):
    ...
    async def _generate_pp_embed(self, interaction: discord.Interaction, target_beatmap: dict, beatmapset_data: dict, beatmap_attributes_response: dict, user_id_for_l10n: str, selected_mods_list: list[str] = None):
        ...
        if cs is None: cs = target_beatmap.get("cs", "N/A")
        ar = attributes_data.get("approach_rate")
        if ar is None: ar = target_beatmap.get("ar", "N/A")
        hp = attributes_data.get("hp_drain") 
        if hp is None: hp = target_beatmap.get("drain", "N/A")
        od = attributes_data.get("accuracy")
        if od is None: od = target_beatmap.get("accuracy", "N/A")
        ...
        if not isinstance(raw_status, str): # If 'status' is not a string (e.g. it's an int or None)
        ...
        status_display_string = get_beatmap_status_display(raw_status, user_id_for_l10n, lambda uid, key, fallback: lstr(uid, key, fallback))
        ...
        if pp_100_raw is None: # Try for any mode if API doesn't provide PP
            ...
            if hasattr(self.osu_api, 'session') and self.osu_api.session is not None:
                ...
                if beatmap_id_for_download:
                    logger.debug(f"[PpCog] API did not return PP. Attempting download and rosu-pp for beatmap ID: {beatmap_id_for_download}")
                    try:
                        osu_file_path = await beatmap_utils.download_osu_file(beatmap_id_for_download, self.osu_api.session)
                        ...
                        logger.debug(f"[PpCog] Using metadata from .osu: {artist} - {title} [{version}]")
                        ...
                        logger.debug(f"[PpCog] rosu-pp calculated: PP={pp_100_raw}, Stars={stars_raw}")
                        used_rosu_pp = True
                        ...
                        logger.error(f"[PpCog] Unexpected error during rosu-pp processing: {type(e).__name__} - {e}")
        ...
        embed.add_field(name=lstr(user_id_for_l10n, "pp_embed_beatmap_status", "Beatmap Status"), value=status_display_string, inline=False)

        embed.add_field(name=lstr(user_id_for_l10n, "pp_embed_attributes", "Attributes"), value=attributes_line, inline=False)
        ...
        embed.add_field(name=lstr(user_id_for_l10n, "pp_embed_map_info", "Map Info"), value=" ".join(map_info_parts), inline=False)
        ...
        if rosu_pp_error_message_key:
            try:
                ...
                if interaction.is_expired():
                    logger.warning(f"[PpCog] Interaction expired, cannot send rosu-pp error followup for key: {rosu_pp_error_message_key}")
                elif interaction.response.is_done():
                     await interaction.followup.send(
                        lstr(user_id_for_l10n, rosu_pp_error_message_key, "An error occurred during local PP calculation."),
                        ephemeral=True
                    )
                else:
                    ...
                    logger.warning(f"[PpCog] Warning: Interaction not yet responded or deferred when trying to send rosu-pp error. Key: {rosu_pp_error_message_key}")
                ...
                logger.error(f"[PpCog] Failed to send rosu-pp error followup message (key: {rosu_pp_error_message_key}): {http_e}")
            except Exception as e:
                logger.error(f"[PpCog] Generic exception sending rosu-pp error followup (key: {rosu_pp_error_message_key}): {e}")

        return embed

    @app_commands.command(name="pp", description="Shows PP information for an osu! beatmap.")
    @app_commands.describe(url="The URL of the osu! beatmap (either beatmapset or specific difficulty).")
    async def pp(self, interaction: discord.Interaction, url: str):
        ...
        match_beatmapset_diff = re.search(r"beatmapsets/(\d+)(?:#(osu|taiko|fruits|mania)/(\d+))?", url)
        ...
        if match_beatmapset_diff:
            ...
            await interaction.followup.send(lstr(user_id_for_l10n, "error_invalid_beatmap_url", "Invalid osu! beatmap URL format."))
            return
        ...
        try:
            ...
            if beatmap_id: # Specific difficulty URL
                if not beatmapset_id: 
                    temp_beatmap_details = await self.osu_api.get_beatmap_details(beatmap_id=beatmap_id)
                    if not temp_beatmap_details or not temp_beatmap_details.get("beatmapset_id"):
                        await interaction.followup.send(lstr(user_id_for_l10n, "error_beatmap_data_incomplete", "Could not retrieve beatmapset ID for the given difficulty."))
                        return
                ...
                if not beatmapset_data or not beatmapset_data.get("beatmaps"):
                    await interaction.followup.send(lstr(user_id_for_l10n, "error_beatmapset_not_found_api", "Could not find the beatmapset or it has no maps."))
                    return

                raw_maps_from_set = beatmapset_data.get("beatmaps", [])
                if not raw_maps_from_set:
                    await interaction.followup.send(lstr(user_id_for_l10n, "error_no_maps_in_set", "Beatmapset contains no difficulties."))
                    return
                ...
                try:
                    ...
                    initial_difficulty_index = next(i for i, bm in enumerate(all_maps_in_set_for_pagination) if bm.get("id") == beatmap_id)
                except StopIteration:
                    logger.warning(f"[PpCog] beatmap_id {beatmap_id} not found in its own sorted beatmapset (ascending sort). Defaulting to index 0.")
                    initial_difficulty_index = 0 # Fallback, though ideally should always be found
                
                if initial_difficulty_index >= len(all_maps_in_set_for_pagination): # Should not happen with StopIteration logic
                ...
                beatmapset_data = await self.osu_api.get_beatmapset(beatmapset_id=beatmapset_id)
                if not beatmapset_data or not beatmapset_data.get("beatmaps"):
                    await interaction.followup.send(lstr(user_id_for_l10n, "error_beatmapset_not_found_api", "Could not find the specified beatmapset or it has no maps."))
                    return
                ...
                if not all_maps_in_set_for_pagination: 
                     await interaction.followup.send(lstr(user_id_for_l10n, "error_no_maps_in_set", "Error processing difficulties in beatmapset."))
                     return

                initial_difficulty_index = 0 # For beatmapset URL, start with the first (easiest after sort)
                target_beatmap = all_maps_in_set_for_pagination[initial_difficulty_index]
                beatmap_id = target_beatmap.get("id") # Update beatmap_id to reflect the initial target
            else:
                # This case should ideally be caught by the initial URL parsing.
                await interaction.followup.send(lstr(user_id_for_l10n, "error_invalid_beatmap_url", "Could not determine beatmap/beatmapset ID from URL."))
                return

            if not target_beatmap or not beatmapset_data or not beatmap_id:
                await interaction.followup.send(lstr(user_id_for_l10n, "error_beatmap_data_incomplete", "Could not retrieve complete beatmap data."))
                return
            ...
            initial_beatmap_attributes = await self.osu_api.get_beatmap_attributes(beatmap_id=beatmap_id, mods=[], ruleset_id=current_ruleset_id)

            if not initial_beatmap_attributes or "attributes" not in initial_beatmap_attributes:
                await interaction.followup.send(lstr(user_id_for_l10n, "error_beatmap_attributes_not_found", "Could not retrieve initial beatmap attributes."))
                return
            ...
            await interaction.followup.send(lstr(user_id_for_l10n, "error_generic_command", f"An unexpected error occurred: {type(e).__name__}"))
