...
MOD_ACRONYMS_TO_BITMASK = {
    ...
    "RX": 128, "HT": 256, "NC": 512, "FL": 1024, "SO": 4096, "PF": 16384,
}
...
def get_beatmap_status_display(status_input, user_id_for_l10n: int, lstr_func) -> str:
    ...
    if isinstance(status_input, str):
        ...
        if normalized_status_str in BEATMAP_STATUS_API_MAP: # Direct match for string keys like "ranked"
    ...
    if status_key == "unknown" and isinstance(status_input, str) and status_input.lower() in BEATMAP_STATUS_EMOJIS:
        status_key = status_input.lower()

    emoji = BEATMAP_STATUS_EMOJIS.get(status_key, BEATMAP_STATUS_EMOJIS["unknown"])
    l10n_key_for_text = BEATMAP_STATUS_L10N_KEYS.get(status_key, BEATMAP_STATUS_L10N_KEYS["unknown"])
    ...
    status_text_localized = lstr_func(user_id_for_l10n, l10n_key_for_text, status_key.replace("-", " ").capitalize())
...
def parse_osu_file_metadata(osu_file_path: str) -> dict:
    ...
    try:
        with open(osu_file_path, "r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                if in_metadata_section:
                    if line.startswith("Title:") and metadata["title"] is None: # Prefer first Title found
                        metadata["title"] = line[len("Title:"):].strip()
                    elif line.startswith("TitleUnicode:") and metadata["title"] is None: # Fallback to TitleUnicode
            ...
            if metadata["title"] is None: metadata["title"] = "Unknown Title"
            if metadata["artist"] is None: metadata["artist"] = "Unknown Artist"
            if metadata["version"] is None: metadata["version"] = "Unknown Version"
        ...
        if metadata["title"] is None: metadata["title"] = "Error Parsing Title"
        if metadata["artist"] is None: metadata["artist"] = "Error Parsing Artist"
        if metadata["version"] is None: metadata["version"] = "Error Parsing Version"
    return metadata
...
async def calculate_pp_with_rosu(
    ...
    try:
        ...
        logger.debug(f"[BeatmapUtils] Calculating PP for: {osu_file_path} with mods {selected_mods}, acc {accuracy}%")
        ...
        if clock_rate != 1.0: # rosu-pp typically handles None clock_rate as 1.0, but explicit is fine
        ...
        perf_attrs = perf_setup.calculate(beatmap_obj) # or calculate(difficulty_attrs) if API expects that
        ...
        logger.debug(f"[BeatmapUtils] Calculated PP: {pp:.2f}, Stars: {stars:.2f}, Max Combo: {max_combo_from_calc}")
...
def delete_osu_file(osu_file_path: str):
    ...
    try:
        if osu_file_path and os.path.exists(osu_file_path) and TEMP_OSU_DIR in osu_file_path: # Safety check
            os.remove(osu_file_path)
            logger.debug(f"[BeatmapUtils] Deleted temporary file: {osu_file_path}")
        else:
            logger.debug(f"[BeatmapUtils] File not deleted (not found or invalid path): {osu_file_path}")
    except Exception as e:
